...
Python ≥3.12, FastAPI 0.139.0, Starlette 1.3.1, uvicorn 0.50.0, SQLAlchemy[asyncio] 2.0.51, asyncpg 0.31.0, Alembic 1.18.5, Pydantic 2.13.4, pydantic-settings 2.14.2, PyJWT[crypto] 2.13.0, bcrypt 5.0.0, structlog 26.1.0. DB = PostgreSQL 17. Pkg mgr = `uv`. Lint = ruff (line 110), types = mypy strict, tests = pytest + pytest-asyncio (`asyncio_mode=auto`).
...
- **Layering:** router (DI aliases only) → service (logic, dataclasses, exceptions) → repository (only ORM). No cross-domain service imports where avoidable; when one module needs another domain's rows, query via its repository, not its service (e.g. M9↔M10).
